...
function me.shiftUpdate()
    ...
    if temp==#joker.jslots or joker.jslots[temp+1].noCopy then
        return
    end
    local copyTarget=joker.jslots[temp+1]
    if copyTarget.bluestorm then
        -- need to update right joker first in this case.
        copyTarget.myjslotid=temp+1
        copyTarget.shiftUpdate()
    end
